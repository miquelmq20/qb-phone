...
local function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Teléfono",
                text = "No tienes ninguna llamada entrante",
                icon = "fas fa-phone",
                color = "#e84118",
            },
        })
...
RegisterKeyMapping('telefono', 'Abrir teléfono', 'keyboard', 'F1')
...
RegisterNUICallback('SharedLocation', function(data)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "WhatsApp",
            text = "La localización ha sido marcada",
...
RegisterNUICallback('PayInvoice', function(data, cb)
    --local sender = data.sender --no funciona, arreglado por zurky
    ...
    TriggerServerEvent('qb-phone:server:setInvoice', true, senderCitizenId)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    ...
    TriggerServerEvent('qb-phone:server:setInvoice', false, senderCitizenId)
...
RegisterNUICallback('track-vehicle', function(data, cb)
    ...
    if findVehFromPlateAndLocate(veh.plate) then
        QBCore.Functions.Notify("El vehículo ha sido marcado en el GPS", "success")
    else
        QBCore.Functions.Notify("No hemos podido localizar el vehículo", "error")
    end
end)
        ...
        if v.name == Name and v.number == Number then
            ...
            --if PhoneData.isOpen then
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Teléfono",
                        text = "Has eliminado el contacto",
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    QBCore.Functions.Notify('Se ha marcado la ubicación del GPS: '..data.alert.title)
...
RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    QBCore.Functions.TriggerCallback('qb-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            ...
            QBCore.Functions.Notify('Estás demasiado lejos de la carrera. Se ha marcado en el GPS la ubicación de esta.', 'error', 5000)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    QBCore.Functions.Notify("Se ha establecido la localización en el GPS (" .. data.HouseData.HouseData.adress .. ")", "success")
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    QBCore.Functions.Notify('Se ha marcado la ubicación en el GPS', 'success')
end)
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
		...
		SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { timeout=3000, title = "Banco", text = "La cuenta bancaria no existe", icon = "fas fa-university", color = "#ff0000", }, })
...
RegisterNetEvent('qb-phone:client:TransferMoney', function(amount, newmoney)
    ...
    SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Banco", text = "&#36;"..amount.." han sido agregados a tu cuenta", icon = "fas fa-university", color = "#8c7ae6", }, })
    ...
    if not delete then -- New Tweet
        if src ~= MyPlayerId then
            SendNUIMessage({
                ...
                PhoneNotify = {
                    title = "Twitter",
                    text = "(@"..NewTweetData.firstName.." "..NewTweetData.lastName..") ha publicado un nuevo tweet",
                    icon = "fab fa-twitter",
                    color = "#1DA1F2",
                },
            })
            ...
            SendNUIMessage({
                ...
                PhoneNotify = {
                    title = "Twitter",
                    text = "El tweet ha sido posteado",
                    icon = "fab fa-twitter",
                    color = "#1DA1F2",
                    timeout = 1000,
                },
        ...
        if src == MyPlayerId then
            SendNUIMessage({
                ...
                PhoneNotify = {
                    title = "Twitter",
                    text = "El tweet ha sido eliminado",
                    icon = "fab fa-twitter",
                    color = "#1DA1F2",
                    timeout = 1000,
                },
...
RegisterNetEvent('qb-phone:client:NewMailNotify', function(MailData)
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Correo",
            text = "Has recibido un nuevo correo de "..MailData.sender,
...
RegisterNetEvent('qb-phone:client:UpdateAdverts', function(Adverts, LastAd)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Anuncios",
            text = "Se ha publicado un nuevo anuncio",
            icon = "fas fa-ad",
            color = "#ff8f1a",
            timeout = 2500,
        },
...
RegisterNetEvent('qb-phone:client:BillingEmail', function(data, paid, name)
    if paid then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Banco de Los Santos',
            subject = 'Factura pagada',
            message = 'La factura fue pagada ('..name..'). Con la cantidad correspondiente de $'..data.amount,
        })
    else
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Banco de los santos',
            subject = 'Factura rechazada',
            message = 'La factura fue rechazada ('..name..') Con la cantidad de $'..data.amount,
        })
    end
end)
        ...
        if PhoneData.isOpen then
            if SenderNumber ~= PhoneData.PlayerData.charinfo.phone then
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "WhatsApp",
                        text = "Tienes un nuevo mensaje de "..IsNumberInContacts(SenderNumber).."",
                        icon = "fab fa-whatsapp",
                        color = "#25D366",
                        timeout = 1500,
                    },
...
RegisterNetEvent('qb-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Banco",
                text = "$"..amount.." han sido retirados de tu cuenta",
                icon = "fas fa-university",
                color = "#ff002f",
                timeout = 3500,
            },
...
RegisterNetEvent('qb-phone:client:GiveContactDetails', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        QBCore.Functions.Notify("¡No hay gente cerca!", "error")
    end
end)
